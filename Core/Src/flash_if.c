...
/**
  ...
  *         1: error occurred
  */
int8_t FLASH_If_Erase(uint32_t StartSector, uint8_t selectedBoot)
{
  uint32_t FlashAddress;
 
  FlashAddress = StartSector;
  uint32_t FlashEndAdress;

  /* Device voltage range supposed to be [2.7V to 3.6V], the operation will
     be done by word */ 
  switch (selectedBoot) {
  	  case 0:
  		  FlashEndAdress = USER_FLASH_BOOT0_LAST_PAGE_ADDRESS;
  		  break;
  	  case 1:
  		  FlashEndAdress = USER_FLASH_BOOT1_LAST_PAGE_ADDRESS;
  		  break;
  	  default:
  		  FlashEndAdress = 0;
  		  break;
  ...
  if (FlashAddress <= FlashEndAdress)
  {
